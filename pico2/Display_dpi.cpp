...
static uint32_t in_scanline_step = 0;
static uint32_t new_scanline_step = 1 << 16;
...
void init_display() {
    ...
    // set pio irq to highest priority
    // need it to happen during line drawing
    irq_set_priority(pio_get_irq_num(pio, 0), PICO_HIGHEST_IRQ_PRIORITY);
}
...
void update_display() {
    if(do_render) {
        ...
        } else if(new_line_width != line_width || new_scanline_step != in_scanline_step) {
...
void set_display_size(int w, int h) {
    if(w < DPI_MODE_H_ACTIVE_PIXELS / 8 || !h)
        return;

    if(w > DPI_MODE_H_ACTIVE_PIXELS)
        w = DPI_MODE_H_ACTIVE_PIXELS;

    new_scanline_step = (h << 16) / DPI_MODE_V_ACTIVE_LINES;
    new_line_width = w;

    // check if we're actually changing scale
    if(new_scanline_step == in_scanline_step && w == line_width)
        return;
